...
const PainPoints = () => {
    const problems = [
        ...
        },
        {
            ...
            solution: "Every alert includes calculated stop-loss levels and position size recommendations based on your account. We cap losses at 6-8% max per trade while letting winners run to 20-50%+ gains. Math that actually works."
        }
    ];
                        ...
                        <GlassCard key={i} className={`p-8 rounded-[2rem] bg-background/50 h-full flex flex-col ${i === 2 ? 'md:col-span-2 lg:col-span-1 md:w-3/4 md:mx-auto lg:w-full' : ''}`}>
                            ...
                            <div className="pt-6 border-t border-white/5 bg-primary/5 -mx-8 -mb-8 p-8 mt-auto">
                                <div className="flex items-start gap-3">
                                    ...
                                    <p className="text-sm font-medium text-white"><span className="font-bold text-primary">💡 Our Solution:</span> {item.solution.replace("We monitor the entire market 24/7. ", "")}</p>
                                </div>
                            </div>
                        </GlassCard>
                    ))}
